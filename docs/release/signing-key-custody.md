...
The updater **public** key is not a secret — it is committed as literal config in `src-tauri/tauri.conf.json` / the release config and compiled into every shipped app so it can verify updater signatures offline.
