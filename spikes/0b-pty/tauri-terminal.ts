// Tauri implementation of the Terminal seam (spikes/0b-pty/terminal-interface.ts).
// Talks to src-tauri/src/pty.rs commands + pty://output/{id} / pty://exit/{id} events.
...
export const tauriTerminal: Terminal = {
  ...
  async onExit(sessionId, handler) {
    const unlisten = await listen<TerminalExitInfo>(`pty://exit/${sessionId}`, (event) => {
      handler(event.payload);
    });
    return unlisten;
  },
